...
//add 
void System::getboundingbox(std::vector< pair<cv::Point2f, cv::Point2f> > &boundingboxes, const double &timestamp)
{	
	mpTracker->boundingboxes= boundingboxes;
	mpTracker->boxTimeStamp=timestamp;
}
//add
void System::getimu(const cv::Mat &imu, const float &imutimestamp)
{	
	mpTracker->imu=imu;
	mpTracker->imutimestamp=imutimestamp;
}
//add
void System::printspeed()
{
	
    vector<KeyFrame*> vpKFs = mpMap->GetAllKeyFrames();
    sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);
    if(vpKFs[vpKFs.size()-3]!=NULL)
    {
		...
		KeyFrame* pKF2 = vpKFs[vpKFs.size()-3];
	   	
		
		cv::Mat t = pKF->GetCameraCenter();
		cv::Mat t2 = pKF2->GetCameraCenter();
		cerr<<"speed:"<< (  3.6*sqrt( (t.at<float>(0)-t2.at<float>(0) )*(t.at<float>(0)-t2.at<float>(0) )+(t.at<float>(1)-t2.at<float>(1) )*(t.at<float>(1)-t2.at<float>(1) )+(t.at<float>(2)-t2.at<float>(2) )*(t.at<float>(2)-t2.at<float>(2) ) )  )/(pKF->mTimeStamp - pKF2->mTimeStamp)<<endl;
	}
...
//add
cv::Mat System::GetCurrentFramePose()
{
    return mpTracker->mCurrentFrame.mTcw;
}
double System::GetTimeStamp()
{
    return mpTracker->mCurrentFrame.mTimeStamp;
}

void System::SaveMap()
{
	const string &filename=mapfile;
